...
test('Validation: Invalid join returns focus to the party code input', async ({ page }) => {
  await page.goto('/');
  await page.waitForTimeout(1000);

  await page.fill('#playerName', 'JoinTester');
  await page.locator('button#multiplayerBtn').click();
  await expect(page.locator('#multiplayerOptions')).toBeVisible();
  await page.locator('button#joinPartyBtn').click();
  await expect(page.locator('#joinPartyDiv')).toBeVisible();

  await page.fill('#partyCodeInput', '123');
  await page.locator('button#joinPartySubmitBtn').click();

  await expect(page.locator('#partyCodeInput')).toBeFocused();
  await expect(page.locator('.notification.error').last()).toContainText(/party code/i);
});
