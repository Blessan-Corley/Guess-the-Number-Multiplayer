...
describe('PartyService', () => {
  ...
  test('Should prevent joining full party', async () => {
    ...
    await expect(partyService.joinParty(party.code, 'socket_p3_789', 'Player3')).rejects.toThrow(
      'Party is full'
    );
  });
  ...
  test('Should return party and player from a single socket context snapshot', async () => {
    const party = await partyService.createParty(hostSocketId, hostName);
    const context = await partyService.getPartyContextBySocket(hostSocketId);

    expect(context).toBeDefined();
    expect(context.party.code).toBe(party.code);
    expect(context.player.id).toBe(party.hostId);
    expect(context.playerId).toBe(party.hostId);
    expect(context.partyCode).toBe(party.code);
  });
});
